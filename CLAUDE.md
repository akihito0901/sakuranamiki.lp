...
- [ ] **9/20に枠が空いたら、溜まったLINE友だちへ声をかける**
  - 9/12にCTAを「予約」訴求から「確認」訴求へ戻した。直近の枠が既存顧客で
    埋まっており、案内できる日程が無いまま予約を促すと期待と実態がずれるため
  - **今のLPは「入口を広げて友だち追加を溜める」役に徹している。**
    予約に変えるのはLINE側の運用であって、LPではない
  - **出口の運用を決めないと、前回と同じ「50件追加・予約ゼロ」になる。**
    作業に入る前にこの段取りが決まっているか確認すること
  - LINEの初回返信に「最短◯月◯日からご案内できます」と具体的な日付を入れる
    案を出してあるが、未実施
